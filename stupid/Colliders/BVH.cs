...
namespace stupid.Volumes
{
    public interface IBVHClientObject
    {
        SBounds GetBounds();
        Vector3S Position { get; }
        Vector3S PreviousPosition { get; }
    }
    ...
    public class BoundingVolumeHierarchy<T> where T : class, IBVHClientObject
    {
        ...
        private readonly f32 AABB_EXTENSION = (f32)0.1f;
        private readonly f32 AABB_MULTIPLIER = (f32)2f;
...
#pragma warning restore IDE0052 // Remove unread private members
        ...
        public int Add(T clientObject)
        {
            ...
            SBounds newBounds = clientObject.GetBounds();
        ...
        public bool Update(T clientObject)
        {
            ...
            SBounds aabb = clientObject.GetBounds();
            var displacement = clientObject.Position - clientObject.PreviousPosition;

            // Extend AABB
            SBounds fatAABB = aabb;
            fatAABB.Expand(AABB_EXTENSION);

            // Predict AABB movement
            Vector3S d = AABB_MULTIPLIER * displacement;

            if (d.x < (f32)0f)
                fatAABB.min += d.x * Vector3S.right;
            else
                fatAABB.max += d.x * Vector3S.right;

            if (d.y < (f32)0f)
                fatAABB.min += d.y * Vector3S.up;
            else
                fatAABB.max += d.y * Vector3S.up;

            if (d.z < (f32)0f)
                fatAABB.min += d.z * Vector3S.forward;
            else
                fatAABB.max += d.z * Vector3S.forward;

            SBounds treeAABB = m_nodes[key].AABB;
            if (treeAABB.Contains(aabb))
            {
                Vector3S r = AABB_EXTENSION * Vector3S.one;

                // The tree AABB still contains the object, but it might be too large.
                // Perhaps the object was moving fast but has since gone to sleep.
                // The huge AABB is larger than the new fat AABB.
                Vector3S min = fatAABB.min - (f32)4 * r;
                Vector3S max = fatAABB.max + (f32)4 * r;

                SBounds hugeAABB = new SBounds(min, max);
        ...
        /// </summary>
        public f32 GetSurfaceAreaRatio()
        {
            if (m_root == NULL_NODE)
                return f32.zero;

            f32 rootArea = m_nodes[m_root].AABB.GetSurfaceArea();

            f32 totalArea = f32.zero;
            for (int i = 0; i < m_nodeCapacity; i++)
            {
        ...
        public void RebuildBottomUp()
        {
            ...
            while (count > 1)
            {
                f32 minCost = f32.maxValue;
                int iMin = -1, jMin = -1;
                for (int i = 0; i < count; ++i)
                {
                    var aabbi = m_nodes[nodes[i]].AABB;

                    for (int j = i + 1; j < count; ++j)
                    {
                        var aabbj = m_nodes[nodes[j]].AABB;
                        var b = aabbi;
                        b.Encapsulate(aabbj);

                        f32 cost = b.GetSurfaceArea();
                        if (cost < minCost)
                        {
                            iMin = i;
                            jMin = j;
                ...
                var newAABB = m_nodes[index1].AABB;
        ...
        public void ShiftOrigin(Vector3S newOrigin)
        {
            for (int i = 0; i < m_nodeCapacity; i++)
            {
        ...
        /// </summary>
        public IEnumerable<Node> EnumerateOverlappingLeafNodes(SBounds bounds)
        {
        ...
        /// </summary>
        public SBounds GetPaddedBounds(int key)
        {
            Debug.Assert(0 <= key && key < m_nodeCapacity);
        ...
        /// Get the padded bounds for an object.
        /// </summary>
        public SBounds GetPaddedBounds(T clientObject)
        {
            Debug.Assert(m_keys.TryGetValue(clientObject, out int key));
        ...
        private void InsertLeaf(int leaf)
        {
            ...
            SBounds leafAABB = m_nodes[leaf].AABB;
            int index = m_root;
            while (!m_nodes[index].IsLeaf)
            {
                ...
                var area = m_nodes[index].AABB.GetSurfaceArea();

                SBounds combinedAABB = m_nodes[index].AABB;
                combinedAABB.Encapsulate(leafAABB);
                f32 combinedArea = combinedAABB.GetSurfaceArea();

                // Cost of creating a new parent for this node and the new leaf
                var cost = f32.two * combinedArea;

                // Minimum cost of pushing the leaf further down the tree
                var inheritanceCost = f32.two * (combinedArea - area);

                // Cost of descending into child1
                f32 cost1;
                if (m_nodes[child1].IsLeaf)
                {
                    SBounds aabb = leafAABB;
                ...
                else
                {
                    SBounds aabb = leafAABB;
                    aabb.Encapsulate(m_nodes[child1].AABB);
                    f32 oldArea = m_nodes[child1].AABB.GetSurfaceArea();
                    f32 newArea = aabb.GetSurfaceArea();
                ...
                else
                {
                    SBounds aabb = leafAABB;
                    aabb.Encapsulate(m_nodes[child2].AABB);
                    f32 oldArea = m_nodes[child2].AABB.GetSurfaceArea();
                    f32 newArea = aabb.GetSurfaceArea();
            ...
            while (index != NULL_NODE)
            {
                ...
                SBounds aabb = m_nodes[child1].AABB;
                aabb.Encapsulate(m_nodes[child2].AABB);
        ...
        private int Balance(int iA)
        {
            ...
            if (balance > 1)
            {
                ...
                if (m_nodes[iF].Height > m_nodes[iG].Height)
                {
                    ...
                    SBounds bounds = m_nodes[iB].AABB;
            ...
            if (balance < -1)
            {
                ...
                if (m_nodes[iD].Height > m_nodes[iE].Height)
                {
                    ...
                    SBounds bounds = m_nodes[iC].AABB;
