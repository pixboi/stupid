...
namespace stupid.Constraints
{
    public struct ContactNormal
    {
        ...
        public ContactNormal(in ContactData data)
        {
            point = data.point;
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public void CalculatePrestep(in RigidbodyData a, in RigidbodyData b, in ContactManifoldSlim manifold, in Vector3S ra, in Vector3S rb)
        {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public void WarmStart(ref RigidbodyData a, ref RigidbodyData b, in Vector3S normal, in Vector3S ra, in Vector3S rb)
        {
            Vector3S warmImpulse = normal * accumulatedImpulse;
            ContactSlim.ApplyImpulse(ref a, ref b, warmImpulse, ra, rb);
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public void SolveImpulse(ref RigidbodyData a, ref RigidbodyData b, in Vector3S normal, f32 bias, in Vector3S ra, in Vector3S rb)
        {
    ...
    public struct ContactFriction
    {
        ...
        public ContactFriction(ContactData data)
        {
            featureId = data.featureId;
        ...
        public void WarmStart(ref RigidbodyData a, ref RigidbodyData b, in Vector3S normal, in Vector3S ra, in Vector3S rb)
        {
            Vector3S warmImpulse = tangent * accumulatedFriction;
            ContactSlim.ApplyImpulse(ref a, ref b, warmImpulse, ra, rb);
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public void SolveFriction(ref RigidbodyData a, ref RigidbodyData b, in f32 friction, in f32 accumulatedImpulse, in Vector3S ra, in Vector3S rb)
        {
            var av = a.velocity + Vector3S.Cross(a.angularVelocity, ra);
            var bv = b.isDynamic ? b.velocity + Vector3S.Cross(b.angularVelocity, rb) : Vector3S.zero;
            var contactVelocity = bv - av;
